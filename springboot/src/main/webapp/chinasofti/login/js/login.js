...
function subLogin(){
	...
	// 异步刷新请求
	$.ajax({
        type: "POST",// 请求类型 (方式) post get  loginController
        url: "/postbar/loginController/login",//向后台发送的请求路径
        async: true,
        data:{
        	userName:usrname.trim(),
        	password:pwd.trim()
       	 },//发送后台的数据  主流的格式  就是json数据
        dataType: "json",// 发送的数据的类型
        ...
        }// 成功的时候  回调函数  当ajax发送请求成功的时候回来被调用的函数
    });
